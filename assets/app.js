...
$(document).on("click", ".gif", function() {
	...
	//console.log("this is the button",$(this)
		//[0].attributes["data-animate"].val();
		var state = $(this).attr('data-state');
	if (state == 'still') {
	//$("img").removeAttr("src", results.data[i].images.fixed_height_still.url);
		
		//singerImage.attr("data-state","animate");
		//var results = response.data;
		//$(this).addClass("animate").attr("src", results[i].images.fixed_height.url);
	
		//$("img").removeClass("still");
		//$("img").addClass("animate").attr("src", results[i].images.fixed_height.url);
	...
	//$(this).switchClass("animate", "still");
